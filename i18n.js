(() => {
  ...
  const paragraphs = {
    ja: {
      '最多 15 项；项目较多时建议通过子指标建立层次结构。': '最大15項です。項目が多い場合は、下位基準を使って階層化してください。',
      '添加后会启用“子指标权重”和“ABC 筛选”页面。': '追加すると「下位基準のウェイト」と「ABC分析」ページが有効になります。',
      '填写可接受的最低标准（如“客户满意度 ≥ 85 分”）。在方案评价时，系统会把它作为一个基准方案加入成对比较。': '許容できる希求基準（例：「顧客満足度 85点以上」）を入力します。代替案評価では基準案として一対比較に追加されます。',
      'ABC 入选的子指标将以此最低期望水平作为基准方案参与方案评价。未填写时，方案评价页会提示补充。': 'ABC分析で選定された下位基準は、この希求水準を基準案として代替案評価に参加します。未入力の場合は評価ページで通知します。',
      ...
      '这里展示所有已添加子指标的权重矩阵。逐对判断“行指标相对于列指标”的重要性。群决策时，页面会按选择的方法聚合每个人的判断。': '追加したすべての下位基準のウェイト行列を表示します。行と列の重要度を一対ずつ判断し、グループの場合は選択した方法で集約します。',
      ...
      '它的特点是先建立与决策有关的层次结构：决策课题位于顶层，评估指标位于中间层，多个候选方案位于底层；当指标需要进一步细分时，可以增加子指标层。随后通过两两比较得到权重，并据此对方案进行综合排序。': 'まず、意思決定課題を最上位、評価基準を中間層、代替案を最下位に置く階層を作ります。必要に応じて下位基準を追加し、一対比較からウェイトを求めて代替案を総合順位付けします。',
      ...
      '如有子指标，先完成子指标权重并在 ABC 页面确认入选项，再进行方案评价。': '下位基準がある場合はウェイトを決定し、ABCページで選定項目を確認してから代替案を評価します。',
      ...
      '经典 Saaty RI 表可覆盖到 15 阶，但指标过多会显著增加判断难度。超过 15 项时，建议通过子指标建立层次结构后分组比较。': '標準的なSaaty RI表は15次まで対応しますが、基準が多いほど判断は難しくなります。15項を超える場合は下位基準で階層化し、グループごとに比較してください。'
      ,'最低期望水平会作为最后一列基准方案；最终排序将包含该基准。': '希求水準は最終列の基準案として追加され、最終順位にも含まれます。'
      ,'请先回到“设置”页，在“各筛选子指标的最低期望水平（可选）”中填写基准。': '先に「設定」へ戻り、「選別された下位基準の希求水準（任意）」で基準を入力してください。'
      ...
      ,'ABC 分析筛选后，仅突出显示入选子指标；未入选项已排除。': 'ABC分析後は選定された下位基準のみを強調し、未選定項目は除外します。'
      ,'已显示子指标层。': '下位基準層を表示しています。'
    },
    en: {
      ...
      ,'请先回到“设置”页，在“各筛选子指标的最低期望水平（可选）”中填写基准。': 'Return to Setup and enter the reference values under “Aspiration level for selected subcriteria (optional)”.'
  ...
  function translateDynamic(value) {
    ...
    const replacements = [
      [/^（(\d+) 个子指标）$/, ja ? '（$1件の下位基準）' : ' ($1 subcriteria)'],
      ...
      [/^仅有一个子指标「(.+)」，自动继承该一级指标的全部权重，无需成对比较。$/, ja ? '下位基準「$1」は1件のみのため、上位基準のウェイトをすべて継承し、一対比較は不要です。' : 'There is only one subcriterion, “$1”, so it inherits the full parent weight and needs no pairwise comparison.'],
      [/^(.+) 的子指标比较$/, ja ? '$1 の下位基準比較' : '$1 — Subcriteria Comparison'],
    ...
    return out
      .replace(/（(\d+) 个子指标）/g, ja ? '（$1件の下位基準）' : ' ($1 subcriteria)')
      .replace(' → 子指标', ja ? ' → 下位基準' : ' → Subcriteria')
